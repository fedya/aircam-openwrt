...
static int find_active_socket(unsigned char *mac) {
	int i;

	for (i = 0; i < sockets_count; ++i) {
	    if (memcmp(mac, sockets[i].mac, ETH_ALEN) == 0 && sockets[i].sockfd) {
       		return i;
	    }
	}
	return -1;
...
static int find_socket_by_idx(unsigned char *mac, int if_idx) {
	int i;
	...
	for (i = 0; i < sockets_count; ++i) {
		...
		if (if_idx < 0) {
		    ...
		    if (sockets[i].device_index == if_idx) {
			return i;
		    }
		}
	    }
...
static void setup_sockets() {
	...
	while ((success = get_macs(insockfd, devicename, MT_INTERFACE_LEN, mac))) {
		if (memcmp(mac, emptymac, ETH_ALEN) != 0 && find_socket_by_idx(mac, sockets[sockets_count].device_index) < 0) {
			int optval = 1;
...
static void user_login(struct mt_connection *curconn, struct mt_mactelnet_hdr *pkthdr) {
	...
	/* Open pts handle */
	curconn->ptsfd = open("/dev/ptmx", O_RDWR);
	curconn->ptsfd = curconn->ptsfd < 0 ? -1 : curconn->ptsfd;
...
static void handle_packet(unsigned char *data, int data_len, const struct sockaddr_in *address) {
	...
	if ((socketnum = find_active_socket(pkthdr.dstaddr)) < 0) {
		return;
	}
	...
	switch (pkthdr.ptype) {
		...
		case MT_PTYPE_PING:
			...
			send_special_udp(&(sockets[socketnum]), MT_MACTELNET_PORT, &pdata);
			break;

		case MT_PTYPE_SESSIONSTART:
			syslog(LOG_DEBUG, "(%d) New connection from %s on %s[%d].", pkthdr.seskey,
			       ether_ntoa((struct ether_addr*)&(pkthdr.srcaddr)), sockets[socketnum].name,
			       sockets[socketnum].sockfd);
			...
			memcpy(curconn->srcip, &(((struct sockaddr_in*)address)->sin_addr), 4);
			curconn->srcport = htons(((struct sockaddr_in*)address)->sin_port);
...
int main (int argc, char **argv) {
	...
	for (i = 0; i < sockets_count; ++i) {
		struct mt_socket *socket = &(sockets[i]);
		syslog(LOG_NOTICE, "Listening on %s [%d] for %16s\n", socket->name, socket->sockfd, ether_ntoa((struct ether_addr *)socket->mac));
	}

...
static bfd_boolean
gldarmelfb_linux_eabi_check_ld_so_conf (const char *name, int force)
{
...
gldarmelfb_linux_eabi_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldarmelfb_linux_eabi_write_build_id_section (bfd *abfd)
{
...
gldarmelfb_linux_eabi_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
	...
	{
	  size = gldarmelfb_linux_eabi_id_note_section_size (abfd, &link_info);
	  if (size == 0)
	    {
	      ...
	      if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		{
		  ...
		  t->after_write_object_contents
		    = &gldarmelfb_linux_eabi_write_build_id_section;
  ...
  for (l = needed; l != NULL; l = l->next)
      ...
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (gldarmelfb_linux_eabi_check_ld_so_conf (l->name, force))
	    break;

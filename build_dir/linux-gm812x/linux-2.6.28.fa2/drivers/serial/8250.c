...
static pmuReg_t regUARTArray[] = {
...
#endif
#if !defined(CONFIG_MTD_NAND_FTNANDC023) // UART2 pinmuxed with NAND controller
  #if CONFIG_SERIAL_8250_RUNTIME_UARTS == 3
    {0x3C,          (0x3 << 1),     (0x3 << 1),     (0x0 << 1),     (0x3 << 1)},   // uart1~2 clk on
  ...
  #endif
#else // reserve pinmux mode for NAND controller, so disable UART2
  #if CONFIG_SERIAL_8250_RUNTIME_UARTS == 3
    {0x3C,          (0x1 << 1),     (0x1 << 1),     (0x0 << 1),     (0x1 << 1)},   // uart1 clk on
    {0x50,          (0x0F << 10),   (0x0F << 10),   (0x05 << 10),   (0x0F << 10)}, // uart1 pinmux
  #endif
  #if CONFIG_SERIAL_8250_RUNTIME_UARTS == 4
    ...
    {0x58,          (0x3331 << 8), 	(0x1 << 8), 		(0x0 << 8), 		(0x3331 << 8)}, // uart2~4 pinmux, uart1 for 626
    {0xB8,          (0x3F << 0),    (0x3F << 0),    (0x1C << 0),    (0x3F << 0)},   // uart0~5 clk on    
#endif //#ifdef CONFIG_PLATFORM_GM8210_M
...
#if defined(CONFIG_PLATFORM_GM8126) && defined(CONFIG_MTD_NAND_FTNANDC023)		
		/* 
		 * In GM8126 platform, UART2 is pinmuxed with NAND controller, 
		 * if NAND controller is enabled, it must disable UART2,
		 * otherwise, NAND controller will operate abnormally.
		 */
		if (i == 2) // disable UART2
			continue;
#endif
...
#ifdef CONFIG_PLATFORM_GM8210_M    
    if(nr_uarts > 4)
    	printk("Warning...GM8210 Master UART number > 4\n");
    	
#ifdef CONFIG_SERIAL_THIRD_IP    
    ftpmu010_write_reg(mac_fd, 0x58, 0, (0x3 << 12));//UART2
    ftpmu010_write_reg(mac_fd, 0xB8, 0, (0x1 << 2));
#endif 
#ifdef CONFIG_SERIAL_FOURTH_IP    
    ftpmu010_write_reg(mac_fd, 0x58, 0, (0x3 << 16));//UART3
    ftpmu010_write_reg(mac_fd, 0xB8, 0, (0x1 << 3));
#endif 
#ifdef CONFIG_SERIAL_FIFTH_IP    
    ftpmu010_write_reg(mac_fd, 0x58, 0, (0x3 << 20));//UART4
    ftpmu010_write_reg(mac_fd, 0xB8, 0, (0x1 << 4));
#endif 
    	
#endif 
#ifdef CONFIG_PLATFORM_GM8210_S    
    if(nr_uarts > 1)
    	printk("Warning...GM8210 slave UART number > 1\n");
#endif    	
#endif
